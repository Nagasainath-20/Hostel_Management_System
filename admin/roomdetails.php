...
<?php
  ...
  $sql="SELECT name,regno,email,phoneno,block,roomno FROM user WHERE gender='male';";
...
<?php
  ...
  require_once('../dbConnect.php');
  $sql="SELECT name,regno,email,phoneno,block,roomno FROM user WHERE gender='female';";
    // $sql="SELECT regno,name,email,phoneno,block,roomno FROM user WHERE ";
  $query=mysqli_query($conn,$sql);
if(mysqli_num_rows($query)>0){
